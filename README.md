...
| F.CD.3 | Tworzenie tabel zawężonych list spółek - funkcja `listCompanies(country, sector, industry,...)` | Funkcja z obligatoryjnymi parametrami tekstowymi nazwy kraju (`country`) i nazwy sektora (`sector`) lub nazwy branży (`industry`)  oraz opcjonalnym parametrem `path` zwraca w formacie tabeli (struktury `data.frame`) listę spółek w tak określonym przekroju wraz ze szczegółami: **nazwami, symbolami, ich sektorami, branżami, walutami kwotowania, liczbami pracowników oraz poziomami kapitalizacji rynkowej przeliczonymi do miliardów Euro**. Domyślnie funkcja wykorzystuje dane dołączone a przy ustaleniu wartości parametru `path` ich wersje zapisane na dysku użytkownika (F.DB.1). | 1 | F |
| F.CD.3.1 | Tworzenie tabel bardziej zawężonych list spółek - funkcja `listCompanies(country, sector, industry, market_cap_thresh)` | Ta sama funkcja, co w **F.CD.3** z dodatkowym, opcjonalnym parametrem liczbowym progowego **poziomu kapitalizacji rynkowej** (`market_cap_thresh`) ten sam rezultat zawęża do spółek przekraczających określony poziom kapitalizacji rynkowej w Euro.  | 1 | F |
...
| Id | Nazwa | Opis | Priorytet | Typ |
| --- | --- | --- | :---: | --- |
| NF.Q.SP.1 | Wydajność biblioteki | Operacje dotyczące przetwarzania matematycznego danych *offline* na komputerze bądź serwerze użytkownika funkcje biblioteki wykonują możliwie najszybciej, z użyciem działań na macierzach. | 1 | pozafunkcjonalne |
...
| NF.Q.P.A.1 | Przystosowawczość | Działanie biblioteki nie jest upośledzane poprzez zmiany sprzętu bądź systemu operacyjnego tak długo, jak wspierają one działanie środowiska uruchomieniowego. | 1 | pozafunkcjonalne |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| TF.CD.1 | Test działania danych wbudowanych | Zbiory testowe zostają wywołane po nazwach, po czym zostają przypisane do zmiennych w środowisku lokalnym. Tester sprawdza, czy wywołanie zmiennych w środowisku powoduje ich wyświetlenie. Potem dokonuje inspekcji rozmiarów danych, aby potwierdzić, czy jest zgodna z opisem w pliku pomocy.  | Pozytywny | 0.3.0 | 2023-12-27 | (brak) | MB |
| TF.CD.1 | Test działania danych wbudowanych | Test j.w. po zmianie zakresu danych (usunięcie spółek norweskich i dodanie pominiętych, indeksowych notowanych w państwach w nowym zakresie). | Pozytywny | 1.0.0 | 2023-12-27 | (brak) | MB |
...
| TF.DB.2 | Test aktualizacji bazy plików na dysku użytkownika | j.w. | 0.9.0 | Pozytywny | 2023-01-02 | (brak) | MB |
| TF.DB.2 | Test aktualizacji bazy plików na dysku użytkownika | Test, jak bazowy, po usunięciu błędu niepoprawnej kalkulacji okresu od ostatniej aktualizacji. | 1.0.0 | Pozytywny | 2023-01-02 | (brak) | RKL |
...
| TF.SS.1 | Test tworzenia tabel struktury sektorowej | j.w. | 0.9.0 | Pozytywny | 2023-01-02 | (brak) | MB |
| TF.SS.1 | Test tworzenia tabel struktury sektorowej | j.w. | 1.0.0 | Pozytywny | 2023-01-02 | (brak) | RKL |
...
| TF.STS.3 | Test tworzenia wykresów porównawczych relatywnych wyników ekonomicznych | j.w. | 0.9.0 | Pozytywny | 2023-01-02 | (brak) | MB |
| TF.STA.1 | Test tworzenia tabel raportów analizy technicznej przekrojów sektorowo-geograficznych | Tester sprawdza adekwatność plików pomocy. Wywołuje funkcję z różnymi zestawami niepoprawnych parametrów. Sprawdza, czy jest prezentowana oczekiwana informacja. Następnie wywołuje funkcję z poprawnymi parametrami, dla danych wbudowanych. Weryfikuje sortowanie alfabetyczne najpierw według branży, potem nazwy spółkil; brak wartości pustych; oczekiwane teksty w kolumnach dotyczących sentymentu technicznego. Sprawdza, czy zbiór spółek jest ten sam, co po wywołaniu tabel zawężonych f-cją `listCompanies()` z odpowiednimi parametrami. Na koniec wywołuje f-cję dla danych zaktualizowanych, podając parametr `path` - oczekiwany rezultat **nie musi być zbieżny** z oceną sentymentu na danych wbudowanych (które są starsze).  | Pozytywny | 1.0.0 | 2024-01-03 | (brak) | RKL |
</details>
